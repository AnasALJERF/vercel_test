...
import { collections, features, steps, testimonials, highlights, IMG } from "@/lib/data";
      ...
      <section className="relative flex min-h-[94vh] items-center overflow-hidden">
        ...
        <div className="container-content relative z-10 pt-24">
          <Reveal className="max-w-2xl">
            <span className="eyebrow">تفاصيل تصنع الفرق</span>
            <h1 className="mt-6 text-display-lg text-bone">
              حوّل منزلك إلى مساحة
              <br />
              <span className="bg-honey-grad bg-clip-text text-transparent">تنبض بالأناقة والفخامة</span>
            </h1>
            <p className="mt-6 max-w-xl text-lg leading-relaxed text-bone/80">
              في صالة شهد للمفروشات لا نكتفي ببيع المفروشات، بل نصنع تجربة متكاملة
              تمنح منزلك شخصية مميزة تجمع بين الفخامة والراحة وجودة التنفيذ. من المجالس
              العربية والخليجية وغرف الضيوف والنوم والسفرة، إلى البرادي والأقمشة الفاخرة،
              نهتم بأدق التفاصيل لنقدم لك فرشاً يليق بمنزلك ويدوم لسنوات.
            </p>
            <div className="mt-9 flex flex-wrap gap-4">
              <a href={waLink()} target="_blank" rel="noopener noreferrer" className="btn-honey">اطلب استشارة مجانية</a>
              <Link href="/collections" className="btn-ghost">اكتشف مجموعاتنا</Link>
            </div>
            <p className="mt-8 text-sm text-taupe">بإدارة أحمد الجرف (أبو هادي)</p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="bg-walnut-veil py-24 md:py-28">
        <div className="container-content grid items-center gap-14 lg:grid-cols-2">
          <Reveal>
            ...
            <h2 className="mt-5 text-display-md text-bone">نؤمن أن جمال المنزل يبدأ من التفاصيل</h2>
            <p className="mt-6 leading-loose text-bone/75">
              منذ سنوات، تساعد صالة شهد للمفروشات العائلات على تصميم منازل تجمع بين
              الأناقة والراحة والجودة. لا نعرض منتجات جاهزة فقط، بل نساعد كل عميل على
              اختيار ما يناسب ذوقه ومساحة منزله ليحصل على نتيجة يشعر بالفخر بها كل يوم.
            </p>
            <p className="mt-4 leading-loose text-bone/75">
              نعتمد على خامات مختارة، وخشب من النوع الأول، ومخامل مستوردة، مع فريق
              متخصص يتابع العمل من الفكرة وحتى التسليم، لأننا نؤمن أن الجودة تظهر في
              التفاصيل الصغيرة قبل الكبيرة.
            </p>
            <Link href="/about" className="btn-ghost mt-8">اكتشف قصة شهد للمفروشات</Link>
          </Reveal>
          ...
          <div className="grid gap-4 md:grid-cols-12 md:gap-5">
            <Reveal className="md:col-span-7">
              <div className="relative h-full overflow-hidden rounded-xl2 border border-bone/10">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={highlights[0].image} alt={highlights[0].title} className="h-full min-h-[280px] w-full object-cover md:min-h-[430px]" />
                ...
                <div className="absolute bottom-7 start-7 end-7">
                  <p className="font-latin text-xs tracking-[0.3em] text-honey">{highlights[0].key}</p>
                  <h3 className="mt-2 text-2xl text-bone">{highlights[0].title}</h3>
                  <p className="mt-1.5 text-sm text-bone/75">{highlights[0].line}</p>
                </div>
              </div>
            </Reveal>
            ...
            <div className="grid gap-4 md:col-span-5 md:gap-5">
              {highlights.slice(1).map((h, i) => (
                <Reveal key={h.key} delay={0.1 + i * 0.08}>
                  <div className="relative overflow-hidden rounded-xl2 border border-bone/10">
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={h.image} alt={h.title} className="h-[205px] w-full object-cover" />
                    ...
                    <div className="absolute bottom-5 start-6 end-6">
                      <p className="font-latin text-xs tracking-[0.3em] text-honey">{h.key}</p>
                      <h3 className="mt-1 text-xl text-bone">{h.title}</h3>
                      <p className="mt-1 text-sm text-bone/75">{h.line}</p>
                    </div>
                  </div>
                </Reveal>
              ))}
      ...
      <section className="py-24 md:py-28">
        <div className="container-content">
          <Reveal className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-5 text-display-md text-bone">كل ما تحتاجه لتأثيث منزلك في مكان واحد</h2>
            <p className="mt-4 text-bone/70">اكتشف مجموعة متكاملة من المجالس، الصالونات، غرف النوم، غرف السفرة، البرادي، والأقمشة الفاخرة، بتصاميم تناسب مختلف الأذواق وخامات تمنحك الجودة التي تستحقها.</p>
          </Reveal>
      ...
      <section className="border-y border-bone/10 bg-walnut/40 py-24">
        <div className="container-content">
          <Reveal className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-5 text-display-md text-bone">رحلة واضحة... من أول زيارة حتى آخر لمسة</h2>
          </Reveal>
      ...
      <section className="py-24">
        <div className="container-content">
          <Reveal className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-5 text-display-md text-bone">ثقة بنيناها مع كل مشروع</h2>
          </Reveal>
        ...
        <div className="container-content flex flex-col items-center text-center">
          <Reveal>
            ...
            <p className="mx-auto mt-8 max-w-xl text-lg leading-relaxed text-bone/75">
              تفاصيل متقنة... وخامات مختارة... ونتيجة تليق بمنزلك.
            </p>
          </Reveal>
        </div>
      </section>
